...
public class ModpackAICommands {
    ...
    private static int handleAICommandNoArgs(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        player.sendSystemMessage(Component.literal("§6[ModpackAI] §fAI 채팅을 시작하려면:"));
        player.sendSystemMessage(Component.literal("§f- /ai <질문> - AI에게 바로 질문하기"));
        player.sendSystemMessage(Component.literal("§f- AI 아이템 우클릭 - GUI 채팅창 열기"));
        player.sendSystemMessage(Component.literal("§f- G키 - AI 채팅 GUI 열기 (클라이언트)"));
    ...
    private static int handleModpackAICommand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        player.sendSystemMessage(Component.literal("§6=== ModpackAI Fabric 모드 ==="));
    ...
    private static int handleHelpCommand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        player.sendSystemMessage(Component.literal("§6=== ModpackAI Fabric 도움말 ==="));
        player.sendSystemMessage(Component.literal("§f"));
        player.sendSystemMessage(Component.literal("§e기본 명령어:"));
        player.sendSystemMessage(Component.literal("§f  /ai <질문> - AI에게 질문하기"));
        player.sendSystemMessage(Component.literal("§f  /ai - AI 사용법 안내"));
        player.sendSystemMessage(Component.literal("§f"));
        ...
        player.sendSystemMessage(Component.literal("§f"));
        player.sendSystemMessage(Component.literal("§7AI 아이템을 우클릭하거나 G키로 GUI를 열 수 있습니다!"));
    ...
    private static int handleStatusCommand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        player.sendSystemMessage(Component.literal("§6=== ModpackAI Fabric 시스템 상태 ==="));
    ...
    // RAG 관련 명령어들은 동일하게 유지 (너무 길어서 생략, 위와 동일한 구조)
        ...
        ModpackAIMod.getInstance().getAIManager()
                ...
                .thenAccept(result -> {
                    if (result.has("success") && result.get("success").getAsBoolean()) {
                        ...
                        player.sendSystemMessage(Component.literal("§c[ModpackAI] ❌ RAG 인덱스 구축 실패: " + error));
                    }
                })
                .exceptionally(throwable -> {
